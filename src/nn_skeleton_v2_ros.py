...
class ModelSkeleton:
  ...
  def __init__(self, mc):
    ...
    # self.ph_raw_image_input = tf.placeholder(
    #     tf.float32, [mc.BATCH_SIZE, mc.IMAGE_raw_HEIGHT, mc.IMAGE_raw_WIDTH, 3],name='image_raw_input'
    # )
    self.ph_raw_image_input = tf.placeholder(
        tf.float32, [mc.BATCH_SIZE, mc.IMAGE_HEIGHT, mc.IMAGE_WIDTH, 3],name='image_raw_input'
    )
    self.ph_ori_image_input = tf.placeholder(
        tf.float32, [mc.BATCH_SIZE, mc.IMAGE_raw_HEIGHT, mc.IMAGE_raw_WIDTH, 3],name='image_ori_input'
    )
    # crop image
    # self.ph_image_input = tf.image.crop_to_bounding_box(self.ph_raw_image_input,0,0,mc.IMAGE_HEIGHT, mc.IMAGE_WIDTH)
    # self.ph_image_input = tf.image.crop_to_bounding_box(self.ph_image_input,0,0,mc.IMAGE_HEIGHT, mc.IMAGE_WIDTH)

    # substract
    BGR_MEANS = np.float32(np.zeros((384, 1248, 3)))
    for ii in range(384):
        for jj in range(1248):
            # BGR_MEANS[ii][jj] = np.array([[np.float32([78, 86, 78])]])
            BGR_MEANS[ii][jj] = mc.BGR_MEANS[0][0]
    t2 = tf.constant(BGR_MEANS)
    self.BGR_MEANS_tensor= tf.reshape(t2,[mc.BATCH_SIZE, mc.IMAGE_HEIGHT, mc.IMAGE_WIDTH, 3])
    # self.ph_image_input = tf.subtract(self.ph_image_input , self.BGR_MEANS_tensor)
