...
from nn_skeleton_v2_ros import ModelSkeleton

class SqueezeDet_v2_ros(ModelSkeleton):
  ...
  def _add_forward_graph(self):
    ...
    # test
    # self.ph_image_input = tf.image.crop_to_bounding_box(self.ph_ori_image_input,208,336,mc.IMAGE_HEIGHT, mc.IMAGE_WIDTH)
    self.ph_image_input2 = tf.subtract(self.ph_image_input, self.BGR_MEANS_tensor)

    conv1 = self._conv_layer(
        'conv1', self.ph_image_input2, filters=64, size=3, stride=2,
        padding='SAME', freeze=True)
